...
class numConverter
{
    static void Main()
    {
        ...
        Console.Write("Enter the number in {0}-based numberal system: \n", firstNumeralSystem);
        long input = Math.Abs(long.Parse(Console.ReadLine()));
        Console.Write("Choose the second numeral system into which you want to convert(in range: 2 <s< 16): ");
